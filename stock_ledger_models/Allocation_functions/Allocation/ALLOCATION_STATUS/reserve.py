...
def reserve(conn,O_status,I_alloc_no): 
    L_func_name = "reserve"
    try:
        with open(r'./stock_ledger_models/Allocation_functions/Allocation/GLOBAL_FILES/reserve_queries.yaml') as fh:
            queries         = yaml.load(fh, Loader=yaml.SafeLoader)
            Q_c_get_approval_disable   = queries['reserve']['Q_c_get_approval_disable']
            Q_get_status               = queries['reserve']['Q_get_status']
            Q_get_release_date         = queries['reserve']['Q_get_release_date']
            Q_alloc_criteria           = queries['reserve']['Q_alloc_criteria']
            Q_get_last_inv_upd         = queries['reserve']['Q_get_last_inv_upd']
            Q_get_last_inv_upd_po      = queries['reserve']['Q_get_last_inv_upd_po']
            Q_get_last_inv_upd_asn     = queries['reserve']['Q_get_last_inv_upd_asn']
            Q_get_last_inv_upd_tsf     = queries['reserve']['Q_get_last_inv_upd_tsf']
            Q_get_item_sel_date        = queries['reserve']['Q_get_item_sel_date']
            Q_get_max_upd_date         = queries['reserve']['Q_get_max_upd_date']
            Q_chck_on_hand_diff        = queries['reserve']['Q_chck_on_hand_diff']
            Q_upd_reclac_ind           =  queries['reserve']['Q_upd_reclac_ind']
            Q_asn_chck_recv_date       =  queries['reserve']['Q_asn_chck_recv_date']
            Q_get_days                 =  queries['reserve']['Q_get_days']
            Q_tsf_chck_rel_date        =  queries['reserve']['Q_tsf_chck_rel_date']
            Q_mark_alloc_torecal       =  queries['reserve']['Q_mark_alloc_torecal']
            Q_chck_recalc_ind          =  queries['reserve']['Q_chck_recalc_ind']
            Q_chck_sku_calc_qty        =  queries['reserve']['Q_chck_sku_calc_qty']
            Q_del_records              =  queries['reserve']['Q_del_records']
            Q_ins_alloc_item_loc       =  queries['reserve']['Q_ins_alloc_item_loc']
            Q_sel_alloc_no             =  queries['reserve']['Q_sel_alloc_no']
            Q_upd_status               =  queries['reserve']['Q_upd_status']
            Q_del_itm_search           =  queries['reserve']['Q_del_itm_search']
            Q_get_sysem_date           =  queries['reserve']['Q_get_sysem_date']
            ...
            if L_rel_date < L_system_date :
                err_msg3 = "Release date cannot be greater than the system_date from calendar_variables table."
            ...
            O_status = 5
            if L_chck_wh == None or L_chck_wh == 'W': #kept condition for Warehouse it should be none or 'W'
            ...
            if L_chck_diff_asgn == 1:
                ...
                conn.commit()
                err_msg4 = "Inventory has been updated. Recalculation required."
                return False,err_msg4  #removed to execute query
            print("checking on hand diff to return false")
            ...
            if len(L_asn_date) != 1 and L_chck_1_asn == 'A':
                ...
                print(O_status,"-","rows_affected: ",mycursor.rowcount)
                conn.commit()
                err_msg5 = "One or more ASN's are being received. Recalculation needed."
            ...
            if len(L_chck_tsf) == 1: 
                ...
                print(O_status,"-","rows_affected: ",mycursor.rowcount)
                conn.commit()
                err_msg6 = "Transfer Allocation cannot be approved as system date is not Transfer Approval Date", L_chck_days ," Day(s)."
            ...
            if L_chck_sku_calc_qty == 0 or L_chck_recalc_ind == 'Y':
                err_msg6 = "Allocation is not ready to reserve"
                return False,err_msg6
            ...
            O_status = 15
            L_call_fun,err_message = create_alloc(conn,O_status,I_alloc_no,'RSV') #making I_status consistency with allocation status
            if L_call_fun == False:
                conn.rollback()
                #err_message =" CREATE_ALLOC function failed"
